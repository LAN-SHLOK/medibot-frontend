...
function stopSpeaking() {
    ...
    // 2. Kill backend-generated audio INSTANTLY
    if (currentPlayingAudio) {
        currentPlayingAudio.pause();
        currentPlayingAudio.muted = true; // Extra safety so no bytes escape
    ...
    if (!isListening) {
        ...
        try {
            ...
            if (data.status === "active") {
                ...
                audioEventSource = new EventSource(`${BACKEND_BRAIN}/api/voice/stream`);
                audioEventSource.onmessage = (event) => {
                    try {
                        const msg = JSON.parse(event.data);
                        if (msg.audio) {
                            playBackendAudio(msg.audio);
                ...
                audioEventSource.onerror = (error) => {
                    ...
                    if (isListening) {
                        ...
                        addBotMessage("🛑 Audio Engine Connection Lost.", "system");
                    }
                };

                playSound('mic');
                addBotMessage("🎤 Audio Engine Online. I am listening continuously through Whisper AI, wait 5 sec before audio engine loads", "system");
            }
        } catch (e) {
...
function playBackendAudio(audioData) {
    ...
    try {
        ...
        // 🟢 FIX: Nullify the old context before making a new one
        if (currentPlayingAudio) {
            currentPlayingAudio.pause();
            currentPlayingAudio.removeAttribute('src'); // Force memory free
            currentPlayingAudio.load();
            currentPlayingAudio = null;
        }
        ...
        currentPlayingAudio.onplay = () => {
            setMode('talking');
            syncAnimationWithAudio(currentPlayingAudio);
        };
...
function syncAnimationWithAudio(audioElement) {
    if (!audioCtx) {
    ...
    try {
        ...
        return; // fallback to CSS animation
    }
    ...
    if (!eyeLeft || !eyeRight || !mouthLine) return; // Prevent crashes if UI is missing

    // Force CSS transitions off while JavaScript is manually animating the physics
    eyeLeft.style.transition = 'none';
    eyeRight.style.transition = 'none';
    mouthLine.style.transition = 'none';

    function draw() {
        if (!currentPlayingAudio) {
            // Reset to default neutral face when audio ends abruptly
            eyeLeft.setAttribute('ry', '17');
            eyeRight.setAttribute('ry', '17');
            mouthLine.setAttribute('d', 'M-20 0 Q0 0 20 0'); // Flat curve
            return;
        }
        ...
        // 🟢 1. EYE SQUINT PHYSICS (Reacts to loud volume)
        // Normal ry is 17. As volume goes up, ry goes down (squint). Max squint is ry=10.
        let squintSquash = Math.max(10, 17 - (avgVolume / 255) * 12);
        eyeLeft.setAttribute('ry', squintSquash.toString());
        eyeRight.setAttribute('ry', squintSquash.toString());

        // 🟢 2. MOUTH OPEN PHYSICS (Bezier Curve pulling down)
        // Normal flat line is Q0 0. As volume goes up, center control point goes down. Max open is Q0 25.
        // We exaggerate the mid-frequencies for speech emphasis
        let midSum = 0;
        for (let i = 10; i < 40; i++) midSum += dataArray[i];
        let midAvg = midSum / 30;

        let mouthOpenness = Math.min(25, (midAvg / 255) * 35);
        mouthLine.setAttribute('d', `M-30 0 Q0 ${mouthOpenness} 30 0`);
    }
...
function resetVoiceBars() {
    // Legacy function. Now used to reset the SVG face to neutral
    const eyeLeft = document.getElementById('eyeLeft');
    const eyeRight = document.getElementById('eyeRight');
    const mouthLine = document.getElementById('mouthLine');

    if (eyeLeft) {
        eyeLeft.style.transition = 'all 0.3s ease';
        eyeLeft.setAttribute('ry', '17');
    }
    if (eyeRight) {
        eyeRight.style.transition = 'all 0.3s ease';
        eyeRight.setAttribute('ry', '17');
    }
    if (mouthLine) {
        mouthLine.style.transition = 'all 0.3s ease';
        mouthLine.setAttribute('d', 'M-20 0 Q0 0 20 0'); // Flat curve again
    }
}
...
async function sendMessage() {
    ...
    try {
        ...
        } // 🟢 FIX: Close if (fileToUpload) block
                ...
                for (const line of lines) {
                    ...
                    try {
                        ...
                        if (chunk.final) {
                            ...
                            // 🟢 FIX: Map risk level to severity score and trigger UI
                            if (chunk.risk) {
                                let score = 2; // Default Medium
                                if (chunk.risk.toUpperCase() === "HIGH") score = 3;
                                else if (chunk.risk.toUpperCase() === "LOW") score = 1;
                                handleSeverity(score);
                            }
    ...
    try {
        ...
        if (data.suggestions && data.suggestions.length > 0) {
            ...
            data.suggestions.forEach(action => {
                ...
                btn.onclick = async () => {
                    // 🟢 FIX: Prevent Race Condition by disabling button immediately
                    btn.disabled = true;
